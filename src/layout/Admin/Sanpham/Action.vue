                    ...
                    <v-form>
                        <v-container>
                            <v-row>
                                ...
                                <v-col cols="12" sm="6">
                                    <v-text-field label="Tên sản phẩm*" required
                                        v-model="formData.SanPhamTen"></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6">
                                    <v-text-field label="Giá bán*" required v-model="formData.SanPhamGia"></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6">
...
export default {
    ...
    watch: {
        currentData: function () {
            this.formData.SanPhamId = this.currentData.sanPhamId;
            this.formData.SanPhamTen = this.currentData.sanPhamTen;
            this.formData.SanPhamHinhAnh = this.currentData.sanPhamHinhAnh;
            this.formData.SanPhamGia = this.currentData.sanPhamGia;
    ...
    methods: {
        ...
        reSetForm() {
            ...
            this.formData.SanPhamHinhAnh = '';
            this.formData.SanPhamGia = '';
            this.formData.SanPhamId = '';
            this.selectedImage= null
        },
        async updateData() {
            this.dialogloading = true;
            try {
                if (this.formData.SanPhamId === '') {
                    this.formData.SanPhamId = 0;
                    this.formData.SanPhamHinhAnh=this.image;
                    const res = await sanphamApi.addData(this.formData);
                    console.log(res.data);
                    this.dialogloading = false;
                    this.AlertSuccess('Thêm mới thông tin thành công');
                } else {
                    this.formData.SanPhamHinhAnh=this.image;
                    const res = await sanphamApi.updateData(this.formData.SanPhamId, this.formData);
                    console.log(res.data);
                    this.dialogloading = false;
